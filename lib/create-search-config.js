...
function convertAndFilters(andFilters) {
    ...
    _.forEach(byAttribute, (filters, attribute) => {
        ...
        if (Array.isArray(f)) andConditions = andConditions.concat(f);
...
module.exports = function(request) {
    ...
    if (request.elasticsearchQuery) {
        /* query has been constructed in resource */
        search.body.query = request.elasticsearchQuery;
    } else if (request.search && request.search.length > 0) {
        let fields = ['_all'];
