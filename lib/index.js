...
class DataSource {
     ...
     * @param {number} config.timeout       Request timeout in milliseconds. Defaults to 15000.
    ...
    constructor(api, config) {
        ...
        this.client = new elasticsearch.Client({
            node: config.node,
            nodes: config.nodes,
            requestTimeout: config.timeout ?? 15_000,
    ...
    prepare(dsConfig) {
        ...
        if (dsConfig?.field_value_factor) {
    ...
    async process(request) {
        ...
        return {
            data: response?.hits?.hits
                ? response.hits.hits.map(({ _id, _source }) =>
                      /* XXX: API-769 */ flattenObjectKeys({ ..._source, _id })
                  )
                : [],
            totalCount: response.hits.total?.value ?? null
        };
    }
    ...
    return Object.entries(obj).reduce(
        (result, [key, value]) => ({
            ...
            ...(value !== null && typeof value === 'object' && !Array.isArray(value)
                ? Object.entries(flattenObjectKeys(value)).reduce(
